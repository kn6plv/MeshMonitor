...
class Summary extends Page {
  ...
  async select() {
    ...
    this.updates = {};
    const chart5min = [
      await this.generateChart({ id: 'valid_m', title: 'Valid', color: 'green', value: async (from, to) => (await DB.validCount(from, to)).count / 10, step: 10, suggestedY: 200, labels: MIN5_AXIS, ticks: 6, units: { label: 'Seconds', scale: 10 } }),
      await this.generateChart({ id: 'duplicate_m', title: 'Duplicate', color: 'blue', value: async (from, to) => (await DB.duplicateCount(from, to)).count / 10, step: 10, suggestedY: 200, labels: MIN5_AXIS, ticks: 6, units: { label: 'Seconds', scale: 10 } }),
      await this.generateChart({ id: 'outoforder_m', title: 'Out Of Order', color: 'salmon', value: async (from, to) => (await DB.outOfOrderCount(from, to)).count / 10, step: 10, suggestedY: 200, labels: MIN5_AXIS, ticks: 6, units: { label: 'Seconds', scale: 10 } }),
      ...
      await this.generateChart2({ id: 'jitter_m', title: 'Max Jitter', color: 'orange', value: async (from, to) => await DB.maxJitterCount(from, to), step: 10, suggestedY: 100, labels: MIN5_AXIS, ticks: 6, units: { label: 'Seconds', scale: 10 } }),
    ];
    const chart1hour = [
      await this.generateChart({ id: 'valid_h', title: 'Valid', color: 'green', value: async (from, to) => (await DB.validCount(from, to)).count / 300, step: 300, suggestedY: 200, labels: HOUR1_AXIS, ticks: 7, units: { label: 'Minutes', scale: 5 } }),
      await this.generateChart({ id: 'duplicate_h', title: 'Duplicate', color: 'blue', value: async (from, to) => (await DB.duplicateCount(from, to)).count / 300, step: 300, suggestedY: 200, labels: HOUR1_AXIS, ticks: 7, units: { label: 'Minutes', scale: 5 } }),
      await this.generateChart({ id: 'outoforder_h', title: 'Out Of Order', color: 'salmon', value: async (from, to) => (await DB.outOfOrderCount(from, to)).count / 300, step: 300, suggestedY: 200, labels: HOUR1_AXIS, ticks: 7, units: { label: 'Minutes', scale: 5 } }),
      ...
      await this.generateChart2({ id: 'jitter_h', title: 'Max Jitter', color: 'orange', value: async (from, to) => await DB.maxJitterCount(from, to), step: 300, suggestedY: 100, labels: HOUR1_AXIS, ticks: 7, units: { label: 'Minutes', scale: 5 } }),
    ];
    const chart1day = [
